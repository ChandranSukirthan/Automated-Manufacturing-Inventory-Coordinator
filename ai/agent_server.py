...
API_BASE_URL = "http://localhost:5070/api/Inventory"
...
def get_consumption_rate(sku: str) -> float:
    ...
    sku_random = random.Random(sku)
    true_daily_consumption = sku_random.uniform(10.0, 50.0)
    
    # Simulate past stock levels starting high and going down with some noise
    start_stock = 2000
    noise_random = random.Random(f"{sku}:noise")
    days = list(range(30))
    past_stock = [
        start_stock - (true_daily_consumption * day) + noise_random.gauss(0, 15)
        for day in days
    ]

    # Calculate the least-squares slope without requiring NumPy or scikit-learn.
    mean_day = sum(days) / len(days)
    mean_stock = sum(past_stock) / len(past_stock)
    denominator = sum((day - mean_day) ** 2 for day in days)
    slope = sum(
        (day - mean_day) * (stock - mean_stock)
        for day, stock in zip(days, past_stock)
    ) / denominator
...
async def send_alert(client: httpx.AsyncClient, item: Dict[str, Any], is_predictive: bool = False):
...
async def predict_inventory_needs(items: List[InventoryItem]) -> Dict[str, Any]:
    """
...
async def extract_data_agent_workflow(request: DataExtractionRequest) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    import uvicorn
    # Run the server on port 8000 (different from backend on 5070)
