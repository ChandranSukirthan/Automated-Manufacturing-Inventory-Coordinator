...
export default function DefectReportsPage() {
  ...
  const inventoryFor = (defect) => {
    ...
    return quarantines.filter((record) => record.defectReportId === defect.id).map((record) => record.inventoryRollId);
  };

  const filtered = defects.filter((defect) => {
    const haystack = [defect.batchId, defect.productType, defect.severity, defect.status, defect.description, ...inventoryFor(defect)].join(' ').toLowerCase();
